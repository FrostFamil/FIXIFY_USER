...
export default class Household extends Component {
  ...
  makeOrder = () => {
    const { userLocation, creator, price, problem, paymentType, serviceType, scheduled, address } = this.state; 
      ...
      if(res){
        getPushTokens(serviceType).then(res => {
          ...
          for (i = 0; i < res.tokens.length; i++) {
            ...
            const body = "New Household Repairing Request";
  ...
  render() {
    return (
      <View>
        ...
        <View style={{backgroundColor: "#98377a", width: width, height: width * 0.27, alignItems: 'center'}}>
            <Image style={styles.serviceImage} imageStyle={styles.serviceImage} source={require('../../assets/Icons/meiset.jpg')}/>
        </View>
